...
class BitmapMasks(BaseInstanceMasks):
    ...
    def __init__(self, masks, height, width):
        ...
        if len(masks) == 0:
            ...
            print("__len__ self.masks", len(self.masks) == 0)
        else:
            ...
            # print(np.stack(masks).shape, "***", height, "*****", width)
            self.masks = np.stack(masks).reshape(-1, height, width)
            # print("np.stack(masks).reshape(-1, height, width)self.masks", len(self.masks) == 0)
    ...
    def rescale(self, scale, interpolation='nearest'):
        ...
        print("rescale self.masks", len(self.masks) == 0)
        if len(self.masks) == 0:
            new_w, new_h = mmcv.rescale_size((self.width, self.height), scale)
            rescaled_masks = np.empty((0, new_h, new_w), dtype=np.uint8)
        else:
            print("rescale self.masks.shape:", self.masks.shape)
            print("scale:", scale)
            rescaled_masks = np.stack([
                mmcv.imrescale(mask, scale, interpolation=interpolation)
                for mask in self.masks
            ])
            print("rescaled_masks.shape",rescaled_masks.shape)
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        # print("padded_masks.shape:", padded_masks.shape)
