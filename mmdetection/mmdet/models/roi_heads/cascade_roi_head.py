...
from mmdet.core import (bbox2result, bbox2roi, bbox_mapping, build_assigner,
                        ...
                        multiclass_nms)
from mmdet.core.bbox.transforms import combine_proposal_score
...
class CascadeRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        det_labels = []
        proposal_score_list = []
        proposal_score_flag = True
        for i in range(num_imgs):

            if proposal_score_flag:
                det_bbox, det_label, proposal_score = self.bbox_head[-1].get_bboxes(
                    rois[i],
                    cls_score[i],
                    bbox_pred[i],
                    img_shapes[i],
            ...
            proposal_score_list.append(proposal_score)
        ...
        proposal_score_imgs = combine_proposal_score(det_bboxes, proposal_score_list, bbox_results)
        ms_pro_core_result['ensemble'] = proposal_score_imgs
        ...
        if self.with_mask:
            if proposal_score_flag:
                results = list(
                    zip(ms_bbox_result['ensemble'], ms_segm_result['ensemble'], ms_pro_core_result['ensemble']))
            else:
                results = list(
