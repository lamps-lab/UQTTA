...
def addHorizontalVerticalLines(img):
    ...
    # draw vertical lines on the image with no lines
    result_vert = img.copy()
    for cntr in contours:
        x1,y1,x2,y2 = cv2.boundingRect(cntr)
        xcenter = x1+x2//2
        cv2.line(result_vert, (xcenter,0), (xcenter, img_height-1), (0, 0, 0), 2)
    ...
    vert = result_vert.copy()  # image with vertical line to draw horizontal line on
    for cntr in contours:
        x1,y1,x2,y2 = cv2.boundingRect(cntr)
        ycenter = y1+y2//2
            
        cv2.line(vert, (0,ycenter), (img_width-1,ycenter), (0, 0, 0), 2)   # draw horizontal lines on the image with
    return vert
